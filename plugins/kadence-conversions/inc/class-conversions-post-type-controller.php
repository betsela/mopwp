...
class Conversion_Post_Type_Controller {
	...
	public function admin_filter_results( $query ) {
		...
		if ( ! ( 'kadence_conversions' === $query->query['post_type'] && isset( $_REQUEST[ self::TYPE_SLUG ] ) ) ) {
			return $query;
		}
	...
	private function render_post_type_column( string $column_name, int $post_id ) {
		if ( 'trigger' !== $column_name && 'type' !== $column_name && 'display' !== $column_name && 'user_visibility' !== $column_name ) {
			return;
		}
		...
		if ( 'type' === $column_name ) {
			echo esc_html( $this->get_type_label( $meta['type'] ) );
		}
