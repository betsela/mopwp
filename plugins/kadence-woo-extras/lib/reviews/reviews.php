...
class Kadence_Advanced_Reviews {
	...
	public function __construct() {
		...
		add_action( 'after_setup_theme', array( $this, 'update_import_action_field' ) );
	...
	*/
	...
	public function enqueue_admin_import_scripts( $hook ) {
		if ( isset( $hook ) && ! empty( $hook ) && 'toplevel_page_ktwoopoptions' === $hook ) {
			wp_enqueue_script( 'kadence-admin-reviews-import-js', KADENCE_WOO_EXTRAS_URL . 'lib/reviews/js/min/kt_admin_woo_reviews-min.js', array( 'jquery' ), KADENCE_WOO_EXTRAS_VERSION, true );
			wp_localize_script(
	...
	/**
	 * Update Convert comments to reviews Button.
	 */
	public function update_import_action_field() {
		if ( class_exists( 'Redux' ) ) {
			$field = Redux::getField( 'kt_woo_extras', 'kt_review_convert' );
			$field['content'] = '<a class="button-primary kt-review-convert" href="#">Convert Reviews</a><div class="convert-info"><p></p></div>';
			Redux::setField( 'kt_woo_extras', $field );
		}
	}
	/**
